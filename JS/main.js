...
function updateCollisions() {
  ...
  const ew  = SPRITES_SIZE(CONFIG, 'enemy',  'w');
  const eh  = SPRITES_SIZE(CONFIG, 'enemy',  'h');
  ...
  playerBullets.forEach(b => {
    enemies.forEach(e => {
      if (collides(
        b.x - pbc.hitOffX, b.y - pbc.hitOffY, pbc.hitW, pbc.hitH,
  ...
  enemyBullets.forEach(b => {
    if (collides(
      b.x - ebc.hitOffX, b.y - ebc.hitOffY, ebc.hitW, ebc.hitH,
      player.x - pw / 2, player.y - ph / 2, pw, ph
    )) {
      b.dead = true;
      triggerGameOver();
